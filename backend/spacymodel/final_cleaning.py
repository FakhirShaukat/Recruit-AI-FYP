...
def main():
    """Main function to clean resume data"""
    import os
    script_dir = os.getcwd()

    # Try to load from extracted_resumes.json first (original format), otherwise use cleaned_resumes.json
    cleaned_file = os.path.join(script_dir, 'backend\spacymodel\cleaned_resumes.json')
    output_file = os.path.join(script_dir, 'backend\spacymodel\final_cleaned_resumes.json')

    print("Loading from cleaned_resumes.json...")
    resumes = load_json(cleaned_file)

    print(f"Loaded {len(resumes)} resumes")
    print("\nCleaning data...")

    cleaned_resumes = clean_resume_data(resumes)

    print(f"\nCleaned {len(cleaned_resumes)} resumes")
    print(f"Removed {len(resumes) - len(cleaned_resumes)} resumes")

    print("\nSaving cleaned data...")
    save_json(cleaned_resumes, output_file)

    print("Done! Cleaned data saved to cleaned_resumes.json")

    # Print summary statistics
    total_skills = sum(len(r['Skills']) for r in cleaned_resumes.values())
    total_education = sum(len(r['Education']) for r in cleaned_resumes.values())
    total_experience = sum(len(r['Experience']) for r in cleaned_resumes.values())
    total_projects = sum(len(r['Project Experience']) for r in cleaned_resumes.values())

    print(f"\nSummary:")
    print(f"  Total Skills entries: {total_skills}")
    print(f"  Total Education entries: {total_education}")
    print(f"  Total Experience entries: {total_experience}")
    print(f"  Total Project Experience entries: {total_projects}")
